...
fake_def(Size) ->
  ...
  #rf_map_segment{area=#rf_area{x=0,y=0, width=Size, height=Size}, terrain=Terrain}.
...
init(#rf_map_segment{area=#rf_area{x=X, y=Y, width=Width, height=Height}=Area} = Segment) 
  ...
  {ok, #rf_map_segment{area=Area, terrain=Tree}};
init(SegmentDef) ->
  {ok, SegmentDef}.
...
get_terrain_at(X, Y, #rf_map_segment{area=#rf_area{x=StartX, y=StartY}, terrain=Terrain}, _From) ->
...
split_segment(#rf_map_segment{area=#rf_area{x=X, y=Y, width=Width, height=Height}, terrain=Terrain}) ->
  {NW, NE, SW, SE} = split_terrain(Terrain, Width, Height),
  SegWidth = Width div 2,
  SegHeight = Height div 2,
  NWSegment = #rf_map_segment{area=#rf_area{x=X, y=Y, width=SegWidth, height = SegHeight}, terrain=NW},
  NESegment = #rf_map_segment{area=#rf_area{x=X + SegWidth, y=Y, width=SegWidth, height = SegHeight}, terrain=NE},
  SWSegment = #rf_map_segment{area=#rf_area{x=X, y=Y+SegHeight, width=SegWidth, height = SegHeight}, terrain=SW},
  SESegment = #rf_map_segment{area=#rf_area{x=X+SegWidth,y=Y+SegHeight,width=SegWidth,height=SegHeight}, terrain=SE},
